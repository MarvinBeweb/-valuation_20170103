...
$save = "";
$reponse = array();
$finalArray = array(); // tableau stockant questions + reponse
for ($i=0; $i < count($tabquestion)-1 ; $i++) {  // Je boucle dans mon tabquestion
	if ($tabquestion[$i] != " "){ // Si dans mon tabquestion il y a pas du vide 
		if($tabtheme[$i-1] != " ")
		{
		array_push($questionType,  $tabtheme[$i-1]);
		}
		...
		if($tabquestion[$i+1] != " "){
			...
			$save .= "\n". 'Date et heure de la réponse :   '. date('Y-m-d-h');
			array_push($reponse,  $save);
			array_push($finalArray, $questionType);
			array_push($finalArray, $reponse);
			$reponse = array();
			$questionType = array();
		}
